...
def getvideo(name):
    ...
    if not os.path.exists(videopath):
        with st.spinner("Creating directory to store resultant video"):
            mode=0o777
    ...
    with st.spinner("Detecting objects from the frames of the uploaded video, labeling frames by drawing bounding boxes around detected objects and writing the labelled frames to the resultant video"):
        time.sleep(5)
    ...
    while vf.isOpened():
        ...
        # to be used in the function VideoWriter
            ...
            writer = cv2.VideoWriter(vid, fourcc, 30,(frame.shape[1], frame.shape[0]), True)
